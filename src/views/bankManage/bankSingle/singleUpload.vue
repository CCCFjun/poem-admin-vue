    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = [
          ...
          '(必填)题目答案(填写对应答案字符)',
          '答案解析',
          '(必填)所属题型(Java填1,C++填2,Android填3,IOS填4,承上启下填10,画中有话填11,百里挑一填12,分秒必争填13)'
        ]
    ...
    handleSuccess({ results, header }) {
      ...
      results.forEach((item, index) => {
        ...
        Object.keys(item).forEach((key) => {
          ...
          if (key === '(必填)选项A(内容必须以字符A加:开头)') {
            single.choiceA = item[key]
          }
          if (key === '(必填)选项B(内容必须以字符B加:开头)') {
            single.choiceB = item[key]
          }
          if (key === '选项C(内容必须以字符C加:开头)') {
            single.choiceC = item[key]
          }
          if (key === '选项D(内容必须以字符D加:开头)') {
            single.choiceD = item[key]
          }
          if (key === '选项E(内容必须以字符E加:开头)') {
            single.choiceE = item[key]
          }
          if (key === '选项F(内容必须以字符F加:开头)') {
            single.choiceF = item[key]
          }
          if (key === '选项G(内容必须以字符G加:开头)') {
          ...
          if (key === '(必填)所属题型(Java填1,C++填2,Android填3,IOS填4,承上启下填10,画中有话填11,百里挑一填12,分秒必争填13)') {
